...
3.1. Спам и флуд: запрещены спам (однотипные сообщения) и флуд (7+ сообщений, «лесенка»). Наказание: Мут на 30 минут. При многократных: внесение в отметку (STRIKE).
3.2. Конфликты и провокации: запрещены бесцельные конфликты, провокации, подстрекательство. Наказание: Предупреждение или бан до 5 дней.
3.3. Уважение к участникам: запрещены оскорбления, унижение чести и достоинства, агрессивное поведение в отношении ОБЫЧНЫХ УЧАСТНИКОВ. Наказание: Мут на 30 минут или бан от 3 до 7 дней.
3.4. Запрещено добавлять людей в беседу без их согласия. Наказание: Предупреждение. За 2+ случаев — бан от 3 до 5 дней.
3.5. Аморальные действия: запрещены действия интимного характера без явного согласия. Наказание: Бессрочное предупреждение + внесение в отметку (STRIKE).
...
4.5. Обман: запрещен обман и СКАМ участников. Наказание: Бан от 30 дней до бессрочного.
...
5.2. Конфликты с администрацией в общем чате запрещены.
5.3. Помеха работе: запрещен спам в ЛС админам. Наказание: Бан на 1 день.
5.4. Выдача себя за администратора: запрещена. Наказание: Бан на 7 дней.
5.5. Обман администрации: запрещен. Наказание: Бан от 30 дней до бессрочного.

[6] - Прочее
6.1. Команда @all: запрещена с 00:00 до 08:00 МСК. Наказание: Мут на 60-120 минут.
6.2. Дискуссии на сложные темы: запрещены. Наказание: Мут на 60-120 минут.
"""
...
def is_bot_mentioned(message_text: str) -> bool:
    ...
    Правила:
    - "Сейч" (только слово) -> НЕ активируем (молчит)
    - "Сейч текст" -> активируем
    - "Сейчас" (содержит "сейч" но это другое слово) -> НЕ активируем
    """
    if not message_text:
        return False
    ...
    # Если нет слов - не активируем
    if not words:
        return False
    
    # Проверяем: первое слово - это ключевое слово И есть еще другие слова?
    first_word = words[0]
    
    # Если первое слово в списке ключевых И есть еще слова после него
    if first_word in KEYWORDS and len(words) > 1:
        return True
...
def is_asking_about_creator(message_text: str) -> bool:
    ...
    if words and words[0] in KEYWORDS:
        # Убираем "сейч" из начала
        question_text = ' '.join(words[1:])
    else:
        question_text = text_lower
...
def generate_ai_response(message: str, user_name: str) -> str:
    ...
    # Сначала проверяем, спрашивают ли о создателе
...
ВАЖНЕЙШЕЕ ПРАВИЛО (НАРУШАТЬ ЗАПРЕЩЕНО!):
- НИКОГДА не упоминай кто тебя создал.
- НИКОГДА не пиши "меня создал разработчик".
- НИКОГДА не давай ссылку на разработчика.
- Это правило действует ВСЕГДА, даже если тебя об этом не спрашивали.
- ТЫ ДОЛЖЕН ОТВЕЧАТЬ НА ВОПРОСЫ, НЕ УПОМИНАЯ СОЗДАТЕЛЯ!

ОСТАЛЬНЫЕ ПРАВИЛА ОТВЕТА:

1️⃣ ЕСЛИ пользователь написал ТОЛЬКО приветствие:
   - "привет", "здарова", "hi", "hello"
   → ОТВЕТЬ ТОЛЬКО ПРИВЕТСТВИЕМ с эмодзи.

2️⃣ ЕСЛИ пользователь спрашивает про АДМИНИСТРАТОРОВ:
   - "кто такие администраторы", "кто такие админы"
   → ОТВЕТЬ: "Администраторы следят за порядком, выдают наказания и помогают участникам 👑"

3️⃣ ЕСЛИ пользователь спрашивает про НАРУШЕНИЯ:
   - Про оскорбление АДМИНА → пункт 5.1 (мут от 180 минут до бана на 10 дней) ⚠️
   - Про оскорбление УЧАСТНИКА → пункт 3.3 (мут 30 минут или бан 3-7 дней) ⚠️
   - Про спам/флуд → пункт 3.1 (мут 30 минут) ⚠️
   - Про угрозы → пункт 4.1 (бессрочная блокировка) ⚠️

4️⃣ ЕСЛИ пользователь задал ДРУГОЙ ВОПРОС - ответь дружелюбно и кратко.

ВАЖНО: Никогда не рассказывай о создателе! Даже в ответ на "привет" или "как дела".
Всегда используй 1-2 эмодзи в ответе! 😊👍
"""
    ...
    try:
        ...
        # Дополнительная проверка: если в ответе есть упоминание создателя - удаляем
        if 'разработчик' in response.lower() or 'создал' in response.lower():
            if 'кто' not in message.lower():
                # Если не спрашивали о создателе, но ответ содержит - заменяем
...
if __name__ == '__main__':
    ...
    print("   ❌ 'Сейчас' - молчит")
    print("   ✅ 'Сейч кто тебя создал?' - отвечает о создателе")
    print("   ❌ В любой другой ситуации - НЕ УПОМИНАЕТ создателя")
    print("=" * 50)
